...
var (
	AliceMeshConf = &meshauth.MeshCfg{
		ConfigLocation: "../testdata/alice",
		//Modules: []*meshauth.Module{
		//	{
...
func TestFull(t *testing.T) {
	...
	_, err := test.NewClientNode("../testdata/alice")
	if err != nil {
		t.Fatal(err)
	}

	bob := appinit.NewResourceStore()
...
func TestURest(t *testing.T) {
	...
	tok1, err := alice.AuthProviders["gsa"].GetToken(ctx, "https://example.com")
	if err != nil {
...
//				if err != nil {
//					log.Printf("RecordError in LDS: %v", err)
//				}
//
//				if false {
//					log.Println(b)
...
func TestNoCA(t *testing.T) {
	...
	bob := test.NewTestNode("../../testdata/bob")
