...
var logger = ilog.Logger("peering")
...
func (ph *peerHandler) reconnect() {
	// Try connecting
	addrs := ph.getAddrs()
	logger.Infow("reconnecting", "peer", ph.peer, "addrs", addrs)

	err := ph.host.Connect(ph.ctx, peer.AddrInfo{ID: ph.peer, Addrs: addrs})
	if err != nil {
		logger.Infow("failed to reconnect", "peer", ph.peer, "error", err)
...
// removed immediately, but connections won't be formed until `RoundTripStart` is called.
...
// stops.
//
// StartListener peer may also be called multiple times for the same peer. The new
...
func (nn *netNotifee) OpenedStream(n network.Network, s network.Stream) {
	log.Println("Peering.OpenedStream", s.ID(), s.Protocol(), s.Conn().RemoteMultiaddr())
}
