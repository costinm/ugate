...
import (
	...
	sts "github.com/costinm/ugate/auth"
)

// TestSTS uses a k8s connection and env to locate the mesh, and tests the token
// generation. Needs to bootstrap a K8S connection and find the project ID and number.
func TestSTS(t *testing.T) {
	...
	j := &sts.JWT{}
	err = json.Unmarshal([]byte(sts.TokenPayload(masterT)), j)
	...
	j = &sts.JWT{}
	err = json.Unmarshal([]byte(sts.TokenPayload(a)), j)
	log.Println(sts.TokenPayload(a), err)
	log.Println(j, err)
}
