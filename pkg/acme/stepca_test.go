...
func TestAcme(t *testing.T) {
	...
	certDER, err := x509.CreateCertificate(rand.Reader, &template, nil, pub, privk)
	if err != nil {
		panic(err)
	}
	...
	h.Provision(rootCA[0], privk, nil, "testdata")

	ht := httptest.NewUnstartedServer(h)

	ht.StartTLS()

	//r, err := http.NewRequest("GET", ht.URL+"/acme/", nil)
	//ht.Client().Do(r)


	client, err := ca.NewClient(ht.URL, ca.WithTransport(ht.Client().Transport))
