...
import (
	...
	wp "github.com/costinm/meshauth/pkg/webpush"
)
...
// Vapid handler - on /push[/VIP]/topic[?params], on the HTTPS handler
//
...
func (mux *Mux) HTTPHandlerWebpush(w http.ResponseWriter, r *http.Request) {
	...
	var ec string
	if pk, ok := mux.Auth.Cert.PrivateKey.(*ecdsa.PrivateKey); ok {
		ec = base64.RawURLEncoding.EncodeToString(pk.D.Bytes())
	}


	dest := parts[2]
	if dest == "" || dest == mux.Auth.Name  {
		ec := wp.NewWebpushDecryption(ec, mux.Auth.PublicKey, SharedWPAuth)
