...
import (
	...
	"github.com/costinm/meshauth/pkg/webpush"
)
...
func TestChrome(t *testing.T) {
	...
	send(t, `{
      "endpoint":"https://android.googleapis.com/gcm/eRUec0FiUOA:APA91bFyWIKZz5QAusYHrIXwINgssqQ-pz9dG4pzOYUoOEvqmbjVicNj8beZUfZUT3rWrbYC3-khWp4hyOGJbj9_4hfAN5dSj5PHrQ7i5fUGiqgt04upfsQP_ACAZLJGs5qnIudrBcyo",
      
       "keys":{
           "p256dh":"BJIzHZoGcIay4RAnAvz5mo0s-zu6na9fJKKCmt1ekBOXHxfOVF05bv2AeKaz6b8XMd4n5QpnWJAnHPjaTJKlJcU",
            "auth":"68zcbmaevQa7MS7aXXRX8Q"}}`)
...
func send(t *testing.T, epjson string) {
	ep := []byte(epjson)
	sub, err := webpush.SubscriptionFromJSON(ep)
	if err != nil {
		t.Error(err)
	}
	...
	wp := tokens.NewVAPID(tokens.RawKeyToPrivateKey(vapidPriv, vapidPub))

	//var ec string
	//if pk, ok := mesh.Cert.PrivateKey.(*ecdsa.PrivateKey); ok {
	//	ec = base64.RawURLEncoding.EncodeToString(pk.D.Bytes())
	//}
	req, err := webpush.NewRequest(sub.Endpoint, sub.Key, sub.Auth, message, 0, wp)
